...
`$ARGUMENTS`로 이슈 번호를 받는다. 예: `/ac-verifier 1`

## 검증 방법

1. GitHub Issue에서 AC 목록을 가져온다 (`gh issue view $ARGUMENTS`)
2. 이슈 문서(`docs/features/tag/issue-$ARGUMENTS.md`)의 AC 커버리지 표도 함께 참조한다
3. 각 AC에 대해:
   - 해당 AC를 검증하는 테스트가 존재하는지 확인
   - 테스트가 AC의 의도를 정확히 반영하는지 판단
   - 구현 코드가 테스트를 올바르게 충족하는지 확인
4. 경계 조건이 빠져있는지 특별히 주의한다
